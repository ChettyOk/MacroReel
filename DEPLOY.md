...
- For accurate ingredients/steps/macros, keep **`ENABLE_MEDIA_PIPELINE=true`** (or delete the var — the Docker image defaults it on). If it is set to `false` on Render, deep extract stays off for auto-imports; users can still check **Deeper extract** once ffmpeg is present. Caption-only import often invents details when the recipe is spoken or on-screen.
